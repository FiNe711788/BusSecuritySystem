...
namespace SafeSystem
{
    class WMIFunc
    {
        public static string GetSerialPort(string name)
        {
            string a;
            string result = MulGetHardwareInfo(HardwareEnum.Win32_PNPEntity, "Name", name);
            if (result != "")
            { 
                a=result.Substring(result.IndexOf("COM"), result.IndexOf(")")-result.IndexOf("COM")).ToString();
                return a;
            }
            else return "";
        }
